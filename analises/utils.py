...
def is_null(x):
    """
    Return True if `x` is some null value (None or np.nan),
    and False otherwise.
    """
    if type(x) == type(None) or (type(x) == float and np.isnan(x)):
        return True
    else:
        return False
...
def digito_verificador_cpf(cpf_first_9):
    ...
    return str(10 * J + K).zfill(2)


def verifica_digitos_cpf(cpf):
    """
    Given the 11 digits of a CPF `cpf_first_9` 
    (int or str), return True if the last 2 
    digits are correct. Input may include 
    dots and dashes.
    """
    ...
    right_digits = digito_verificador_cpf(cpf[:-2])
    data_digits  = cpf[-2:]

    return right_digits == data_digits
