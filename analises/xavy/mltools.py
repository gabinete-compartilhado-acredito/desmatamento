...
class LogPlus(BaseEstimator, TransformerMixin):
    """
    Transformer that returns the logarithm of 
    `offset_ + X`.
    
    Parameters
    ----------
    
    add_offset : float, array of floats or 'auto'
        Offset to be added to `X` before taking 
        the log. This is meant to avoid negative
        or null (zero) arguments. If 'auto', use
        `1 - X.min(axis=0)` as offset so that the 
        minimum values of transformed `X` are 
        zero.
        
    base10 : bool
        If True, set the logarithm base to 
        10. Otherwise, use the natural log.
   
    apply : bool
        If False, just pass `X` through as in 
    ...
    def __init__(self, add_offset=0, base10=True, apply=True):
        self.apply   = apply
        self.base10  = base10
    ...
    def fit(self, X, y=None):
        """
        If `add_offset` parameter is 'auto', compute 
        the offset for each column for which the 
        minimum log is zero. Otherwise, do nothing.
        ...
        if type(self.add_offset) == str and self.add_offset == 'auto':
            self.offset_ = 1.0 - np.nanmin(X, axis=0)
        else:
            self.offset_ = self.add_offset
    ...
    def transform(self, X):
        """ 
        Compute and return the logarithm of
        the offset plus `X` if `apply` is 
        ...
        X_transformed : array, shape (n_samples, n_features)
            The array containing the element-wise logarithmic 
            (with offset) of the values in `X`.
        """
        ...
        if self.base10:
            return np.log10(self.offset_ + X)
        else:
            return np.log(self.offset_ + X)
 

    def _more_tags(self):
        """
        This function returns sklearn tags informing
        the non-default properties of this estimator.
        """
        return {'allow_nan': True}


# To verify that the estimator is safe and sound:
#l = LogPlus(base10=False, add_offset='auto')
#for estimator, check in check_estimator(l, generate_only=True):
...
def make_numerical_preprocessing(log_transform=False, log_add=0, std_mean=False, std_dev=False, imputer='mean', imputer_indicator=False):
    """
    ...
    log_transform : bool
        Wether or not to transform `X` with a 
        logarithmic function.
    
    log_add : float, array of floats or 'auto'
        offset to be added to `X` before applying
        the log function. If array, apply a 
        different offset to each column. If 'auto',
        compute the offsets such that the smallest
        log in each column is one.
    
    std_mean : bool
        Whether `X` should ve centered.
    
    std_dev : bool
        Whether `X` should be scaled to
        have std. dev. equal to 1.
    
    imputer : str or float
        The strategy to be used by the imputer.
        It can be 'mean', 'median', 'most_frequent'
        or a constant value.
    
    imputer_indicator : bool
        Whether or not to include an extra column 
        stating whether the value was missing before
        imputation or not.
    
    Returns
    -------
    
    The returned object is a scikit-learn
    Pipeline with steps 'log', 'std' and 'imp'.
    """
    
    # Instantiate imputer:
    if type(imputer) != str:
        imp = SimpleImputer(strategy='constant', fill_value=imputer, add_indicator=imputer_indicator)
    else:
        imp = SimpleImputer(strategy=imputer, add_indicator=imputer_indicator)

    # Instantiate standarization:
    std = StandardScaler(with_mean=std_mean, with_std=std_dev)

    # Instantiate log transform:
    log = LogPlus(log_add, apply=log_transform)
    
    return Pipeline([('log', log), ('std', std), ('imp', imp)])
...
def print_cv_scores(estimator, X, y, scorers, cv=5):
    """
    Print the mean and its std. dev. of cross-validated
    metrics.
    
    Parameters
    ----------
    estimator : sklearn Estimator
        The model which to evaluate using 
        cross-validation.
    X : array-like (2D)
        The data's features.
    y : array-like (1D)
        The data's targets.
    scorers : list of str or callables
    ...
    for scorer in scorers:
        ...
        #scores = [scorer(y[i * n_instances : (i + 1) * n_instances], y_pred[i * n_instances : (i + 1) * n_instances]) for i in range(cv)]
        mean, dev = mean_dev_scores(scores)
        print('{}: {:.4f} +/- {:.4f}'.format(scorer, mean, dev / np.sqrt(len(scores))))
